...
These come from the Pi runtime or Aligned Agent OS extensions. No extra setup is needed unless a note says otherwise.
...
Aligned Agent OS includes a lightweight voice skill:

```text
$ALIGNED_AGENT_HOME/agent/skills/voice/
```
...
The operator can also speak into ChatGPT, Grok, Gemini, or another voice interface and paste the transcript into Aligned Agent OS. Whisper is optional for local/offline STT later.
